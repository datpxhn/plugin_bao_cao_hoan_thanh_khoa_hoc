...
$PAGE->set_title('E-learning TLU');
$PAGE->set_heading('E-learning TLU');
echo $OUTPUT->header();

$courses = $DB->get_records_sql('SELECT * FROM {course} WHERE summaryformat=?', ['1']);
foreach ($courses as $course) {
	$id = $course->id;
	echo '<h2>' . $course->fullname . "</h2></br>";
	if (!$course) {
...
// if ($csv && $grandtotal && count($activities) > 0) {
	// 	// Only show CSV if there are some users/actvs
